...
function onShowReport ()
{
	...
	var entity = parseInt ($ ('#sel-entity').val (), 10);
	if (since == '')
	{
	...
	if (isNaN (entity) || entity < 1)
	{
		$ ('#p-error').text ('Elija una entidad.');
		$ ('#p-error').show ();
		return;
	}
	window.open ('../reports/workerFeeEntity.html?e=' + entity + '&s=' + encodeURI (since) + '&u=' + encodeURI (until) );
}
...
function onPDF ()
{
	...
	var params = {'s': since, 'u': until, 'e': entity};
	...
	var jqXHR = $.ajax ( {
		...
		type: 'POST',
		data: JSON.stringify ( {'f': 'repGenWorkerFeeEntityPDF', 'p': params} ),
...
function onCSV ()
{
	...
	var jqXHR = $.ajax ( {
		...
		contentType: 'application/json',
		type: 'POST',
		data: JSON.stringify ( {'f': 'repGenWorkerFeeEntityCSV', 'p': params} ),
		error: function (jqXHR, textStatus, errorThrown)
		{
			onAjaxError (jqXHR, textStatus, errorThrown);
		},
