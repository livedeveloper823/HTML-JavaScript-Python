...
function onShowReport ()
{
	...
	window.open ('../reports/systemLog.html?s=' + encodeURI (since) + '&u=' + encodeURI (until) );
}
...
function onPDF ()
{
	...
	var jqXHR = $.ajax ( {
		...
		type: 'POST',
		data: JSON.stringify ( {'f': 'repGenSystemLogPDF', 'p': params} ),
...
function onCSV ()
{
	...
	var jqXHR = $.ajax ( {
		...
		contentType: 'application/json',
		type: 'POST',
		data: JSON.stringify ( {'f': 'repGenSystemLogCSV', 'p': params} ),
		error: function (jqXHR, textStatus, errorThrown)
		{
			onAjaxError (jqXHR, textStatus, errorThrown);
		},
