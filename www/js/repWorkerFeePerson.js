...
function onShowReport ()
{
	...
	window.open ('../reports/workerFeePerson.html?c=' + encodeURI (cui) );
}
...
function onPDF ()
{
	...
	var jqXHR = $.ajax ( {
		...
		type: 'POST',
		data: JSON.stringify ( {'f': 'repGenWorkerFeePersonPDF', 'p': params} ),
...
function onCSV ()
{
	...
	var jqXHR = $.ajax ( {
		...
		contentType: 'application/json',
		type: 'POST',
		data: JSON.stringify ( {'f': 'repGenWorkerFeePersonCSV', 'p': params} ),
		error: function (jqXHR, textStatus, errorThrown)
		{
			onAjaxError (jqXHR, textStatus, errorThrown);
		},
