...
function init ()
{
	...
	var jqXHR = $.ajax ( {
		...
		type: 'POST',
		data: JSON.stringify ( {'f': 'repGenWorkerFeeMonth', 'p': {'y': year, 'm': month} } ),
...
function onGenerateSuccess (jqXHR, textStatus, data, requestCode)
{
	...
	$ ('#td-who').text (meta [1] );
	$ ('#td-year').text (meta [2] );
	$ ('#td-month').text (meta [3] );
	var trs = '';
	for (var idx in data)
	{
		var row = data [idx];
		trs += '<tr><td>' + escape (row [0] ) + '</td><td>' + escape (row [1] ) + '</td><td class="currency">Q ' + escape (row [2] ) + '</td></tr>';
	}	
