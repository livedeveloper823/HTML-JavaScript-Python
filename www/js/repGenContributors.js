...
function init ()
{
	var jqXHR = $.ajax ( {
		...
		type: 'POST',
		data: JSON.stringify ( {'f': 'repGenContributors', 'p': {} } ),
...
function onGenerateSuccess (jqXHR, textStatus, data, requestCode)
{
	...
	{
		var row = data [idx];
		trs += '<tr><td>' + escape (row [0] ) + '</td><td>' + escape (row [1] ) + '</td><td>' + escape (row [2] ) + '</td><td>' + escape (row [3] ) + '</td><td>' + escape (row [4] ) + '</td></tr>';
	}	
	trs += '<tr class="tr-sum"><td colspan="2">Total</td><td>' + escape (total) + '</td><td>' + escape (total2) + '</td><td></td></tr>';
