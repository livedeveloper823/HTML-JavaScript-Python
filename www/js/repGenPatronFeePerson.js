...
function init ()
{
	...
	var jqXHR = $.ajax ( {
		...
		type: 'POST',
		data: JSON.stringify ( {'f': 'repGenPatronFeePerson', 'p': {'c': cui} } ),
...
function onGenerateSuccess (jqXHR, textStatus, data, requestCode)
{
	...
	{
		var row = data [idx];
		trs += '<tr><td>' + escape (row [0] ) + '/' + escape (row [1] ) + '</td><td>' + escape (row [2] ) + '</td><td>' + escape (row [3] ) + '</td><td class="currency">Q ' + escape (row [4] ) + '</td><td class="currency">Q ' + escape (row [5] ) + '</td><td>' + escape (row [6] ) + '</td></tr>';
	}	
	trs += '<tr class="tr-sum"><td colspan="3">Total</td><td class="currency">Q ' + escape (total1) + '</td><td class="currency">Q ' + escape (total2) + '</td><td></td></tr>';
	$ ('#theBody').html (trs);
}
