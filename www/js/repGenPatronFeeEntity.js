...
function init ()
{
	...
	var jqXHR = $.ajax ( {
		...
		type: 'POST',
		data: JSON.stringify ( {'f': 'repGenPatronFeeEntity', 'p': {'s': since, 'u': until, 'e': entity} } ),
...
function onGenerateSuccess (jqXHR, textStatus, data, requestCode)
{
	...
	{
		var row = data [idx];
		trs += '<tr><td>' + escape (row [0] ) + '/' + escape (row [1] ) + '</td><td>' + escape (row [2] ) + '</td><td class="currency">Q ' + escape (row [3] ) + '</td><td>' + escape (row [4] ) + '</td></tr>';
	}	
