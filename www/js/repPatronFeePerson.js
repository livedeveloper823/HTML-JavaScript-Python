...
function onShowReport ()
{
	...
	window.open ('../reports/patronFeePerson.html?c=' + encodeURI (cui) );
}
...
function onPDF ()
{
	...
	var jqXHR = $.ajax ( {
		...
		type: 'POST',
		data: JSON.stringify ( {'f': 'repGenPatronFeePersonPDF', 'p': params} ),
...
function onCSV ()
{
	...
	var jqXHR = $.ajax ( {
		...
		contentType: 'application/json',
		type: 'POST',
		data: JSON.stringify ( {'f': 'repGenPatronFeePersonCSV', 'p': params} ),
		error: function (jqXHR, textStatus, errorThrown)
		{
			onAjaxError (jqXHR, textStatus, errorThrown);
		},
