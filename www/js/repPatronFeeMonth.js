...
function onShowReport ()
{
	...
	window.open ('../reports/patronFeeMonth.html?y=' + year + '&m=' + month);
}
...
function onPDF ()
{
	...
	var jqXHR = $.ajax ( {
		...
		type: 'POST',
		data: JSON.stringify ( {'f': 'repGenPatronFeeMonthPDF', 'p': params} ),
...
function onCSV ()
{
	...
	var jqXHR = $.ajax ( {
		...
		contentType: 'application/json',
		type: 'POST',
		data: JSON.stringify ( {'f': 'repGenPatronFeeMonthCSV', 'p': params} ),
		error: function (jqXHR, textStatus, errorThrown)
		{
			onAjaxError (jqXHR, textStatus, errorThrown);
		},
